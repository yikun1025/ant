...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        nearest_place = None
        transitions = 0
        while check_place is not hive and nearest_place is None and transitions <= self.max_range:
            if len(check_place.bees) == 0 or transitions < self.min_range:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        self.disgesting = self.time_to_digest
        # END Problem 10

    def action(self, colony):
        # BEGIN Problem 10
        bee = list(self.place.bees)
        # print("digesting", self.disgesting)
        # print("bees remaning", self.place.bees)
        digesting0 = self.disgesting == 0 and self.place.bees is not None
        if self.disgesting != 0:
            self.disgesting -= 1
        ...
        if self.impostor:
            ...
            def double_damage(ant):
                if ant not in self.doubled:
                    ant.damage *= 2
                    QueenAnt.doubled.append(ant)

            self.throw_at(self.nearest_bee(colony.hive))
            current = self.place.exit
            while current:
                if current.ant:
                    if hasattr(current.ant, 'ant') and current.ant.ant:
                        double_damage(current.ant.ant)
                    double_damage(current.ant)
...
def apply_effect(effect, bee, duration):
    ...
    def new_action(colony):
        ...
        if counter < duration:
            counter += 1
            effect(old_action, colony)(colony)  # bee.action = effect(bee.action, colony)
        else:
            old_action(colony)  # bee.action = bee.old_action
